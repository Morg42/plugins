...
class TestSECallerIgnored(_Base):
    ...
    def test_se_plugin_caller_trailing_instance(self):
        ...
        result = self.call(
            caller='StateEngine Plugin instance1',
            source=self.path,
            value=False,
        )
...
class TestExcludeList(_Base):
    ...
    def setUp(self):
        ...
        _register_item(
            self.sh, self.path, value=False,
            conf={'se_manual_exclude': 'autoblind'},
        )
    ...
    def test_exclude_regex_pattern(self):
        ...
        _register_item(
            self.sh, 'regex.item', value=False,
            conf={'se_manual_exclude': 'auto.*'},
        )
    ...
    def test_exclude_list_as_python_list(self):
        ...
        _register_item(
            self.sh, 'list.item', value=False,
            conf={'se_manual_exclude': ['knx', 'zigbee']},
        )
        self.fn.ab_alive = True
...
class TestIncludeList(_Base):
    ...
    def test_include_list_multiple_entries(self):
        ...
        _register_item(
            self.sh, 'multi.item', value=False,
            conf={'se_manual_include': ['knx', 'mqtt']},
        )
        self.fn.ab_alive = True
    ...
    def test_include_beats_exclude(self):
        ...
        _register_item(
            self.sh, 'both.item', value=False,
            conf={
                'se_manual_exclude': 'knx',
                'se_manual_include': 'knx',
            },
        )
        self.fn.ab_alive = True
    ...
    def test_no_flip_before_alive(self):
        ...
        for caller in ('knx', 'mqtt', 'Admin', 'Eval'):
            with self.subTest(caller=caller):
                result = self.fn.manual_item_update_eval(
                    self.path, caller, 'some.source'
                )
                self.assertIs(result, False,
                              f"caller={caller!r} should not flip during startup")
