...
class modbus_tcp(SmartPlugin):
    ...
    def __read_Registers(self, regPara: dict):
        ...
        try:
            ...
            self.logger.error(
                f"unable to unpack data for datatype={dataType.lower()} for read {objectType}.{address}.{slaveUnit} (address.slaveUnit) regCount:{registerCount}"
            )
            raise ModbusException(
                f"Exception: unable to unpack data for datatype={dataType.lower()} for read {objectType}.{address}.{slaveUnit} (address.slaveUnit) regCount:{registerCount}"
            )
